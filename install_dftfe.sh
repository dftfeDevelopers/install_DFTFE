...
c_flags="-fPIC -O2 -xCORE-AVX512 -qopenmp -fp-model=precise"

cxx_compiler=icpx
mpi_cxx_compiler=mpicxx
cxx_flags="-fPIC -O2 -xCORE-AVX512 -qopenmp -fp-model=precise"

fortran_compiler=ifx
mpi_fortran_compiler=mpifort
fortran_flags="-fPIC -O2 -xCORE-AVX512 -qopenmp -assume dummy_aliases -fp-model=precise"

device_compiler=mpicxx
device_flags="--intel -fsycl -O2 -fsycl-targets=spir64_gen -Xsycl-target-backend \"-device pvc\" -fp-model=precise"
...
numdiffDir=""
dcclDir=""
blasLapackFlags="-qmkl=parallel"
scalapackFlags="-lmkl_scalapack_lp64 -lmkl_blacs_intelmpi_lp64"
...
if [[ $downloadDependencies ]]; then
  ...
  if [ -z $alglibDir ]; then
    wget -N -q --show-progress https://www.alglib.net/translator/re/alglib-4.05.0.cpp.gpl.tgz &
  fi
  if [ -z $libxcDir ]; then
  ...
  if [ -z $elpaDir ]; then
    wget -N -q --show-progress https://elpa.mpcdf.mpg.de/software/tarball-archive/Releases/2025.01.002/elpa-2025.01.002.tar.gz
  fi
  wait
  cd $dftfeDir
  if [ -z "$( ls -A 'src' )" ]; then
    git clone --branch publicGithubDevelop https://bitbucket.org/dftfedevelopers/dftfe.git src
  else
    cd src
    git pull
    cd ..
...
if [[ $alglibDir ]]; then
  ...
  if [[ $installAlglib ]]; then
    ...
    tar xzf alglib-4.05.0.cpp.gpl.tgz --checkpoint=.100
    echo "Extraction done"
...
if [[ $dealiiDir ]]; then
  ...
  dealiiDir=$dependencyDir/install/dealii
  if [[ $installdealii ]]; then
    ...
    cd dealii-9.6.2
    mkdir -p build && cd build
    cmake -DCMAKE_CXX_STANDARD=17 -DCMAKE_CXX_FLAGS="$cxx_flags -std=c++17" -DCMAKE_C_FLAGS="$c_flags" -DDEAL_II_ALLOW_PLATFORM_INTROSPECTION=OFF -DDEAL_II_FORCE_BUNDLED_BOOST=OFF -DDEAL_II_WITH_TASKFLOW=OFF -DKOKKOS_DIR=$kokkosDir -DBOOST_DIR=$boostDir -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=$mpi_c_compiler -DCMAKE_CXX_COMPILER=$mpi_cxx_compiler -DCMAKE_Fortran_COMPILER=$mpi_fortran_compiler -DDEAL_II_WITH_TBB=OFF -DDEAL_II_COMPONENT_EXAMPLES=OFF -DDEAL_II_WITH_MPI=ON -DDEAL_II_WITH_64BIT_INDICES=ON -DP4EST_DIR=$p4estDir -DDEAL_II_WITH_LAPACK=ON -DCMAKE_INSTALL_PREFIX=$dealiiDir ..
    cmake --build . -j$nprocs
    cmake --install .
  fi
  echo "dealii path set to: $dealiiDir"
...
if [[ $elpaDir ]]; then
  ...
  if [[ $installElpa ]]; then
    ...
    tar xzf elpa-2025.01.002.tar.gz --checkpoint=.100
    echo "Extraction done"
    echo "Compiling ELPA"
    cd elpa-2025.01.002
    mkdir -p build && cd build
    CC=$mpi_c_compiler FC=$mpi_fortran_compiler CXX=$mpi_cxx_compiler CFLAGS="$c_flags" FCFLAGS="$fortran_flags" CXX_FLAGS="$cxx_flags" SCALAPACK_FCFLAGS="$blasLapackFlags $scalapackFlags" SCALAPACK_LDFLAGS="$blasLapackFlags $scalapackFlags" ../configure --prefix=$elpaDir --enable-avx-kernels --enable-avx2-kernels --enable-avx512-kernels --enable-c-tests=no --enable-cpp-tests=no --enable-option-checking=fatal --enable-sse-kernels --enable-sse-assembly-kernels --with-mpi --disable-static --enable-shared --disable-silent-rules
    make -j$nprocs
    make install
  fi
  echo "ELPA path set to: $elpaDir"
